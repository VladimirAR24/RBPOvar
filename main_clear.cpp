...
bool parsePayloadSafe(const FrameHeader& header,
    ...
    if (payload.size() != expectedBytes) {
        std::cerr << "Error: payload size does not match expected byte count\n";
        return false;
    }
...
bool parsePayloadDemoFixed(const FrameHeader& header,
                           const std::vector<char>& payload) {
    // Исправленная демонстрационная ветка.
    // Раньше здесь использовались malloc(), ручное копирование и чтение values[3]
    // без проверки количества элементов. Теперь память управляется std::vector,
    // а обращение к элементам выполняется только после проверки размера.
    ...
    if (values.size() >= 4) {
        ...
        std::cout << "  demo value[3]: not available, sensor count is "
                  << values.size() << "\n";
    }
...
#ifdef DEMO_BUGS
        if (!parsePayloadDemoFixed(header, payload)) {
            return 5;
        }
#else
        std::vector<int16_t> values;
